...
# if we are calling this from the install command in ebagis.py,
# this import will fail with the error "attempted a relative
# import in a non-package" because we will have imported the
# install module directly, without the ebagis package being
# loaded, so we add the utils to the path in ebagis.py and
...
def load_conf_file(conf_path=CONF_FILE):
    try:
        ...
        raise Exception((
            ...
            'Have you installed this ebagis instance?'
        ).format(conf_path))
...
# project as ebagis.py).
class Install(object):
    help = "Install this ebagis instance to the local system."
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument(
            '--version',
            action='store_true',
            help='Display the version of ebagis to be installed then exit.',
        )
        parser.add_argument(
            '-v',
            '--verbosity',
        ...
        parser.add_argument(
            '-n',
            '--instance-name',
            help=('The name of the ebagis instance. '
                  'Default the name of the directory '
                  'containing this django-ebagis instance.'),
        )
        parser.add_argument(
            '--domain-name',
            help=('The domain name of this ebagis instance. '
        ...
        parser.add_argument(
            '-U',
            '--celery-broker-user',
            help=('The name of the message broker user. '
                  'Default is the instance name.'),
        )
        parser.add_argument(
            '-P',
            '--celery-broker-password',
            help=('Password for the specified broker user. '
                  'Default is to prompt user for input.'),
        )
        parser.add_argument(
            '-k',
    ...
    def extract_settings_from_options(self, options):
        ...
        settings['CELERY_BROKER_USER'] = get_default(
            options,
            'celery_broker_user',
            settings['INSTANCE_NAME'],
        )
        settings['CELERY_BROKER_PASSWORD'] = get_default(
            options,
            'celery_broker_password',
            get_password('Please enter the broker user password: '),
        )
    ...
    def gen_arc_pth_file(self):
        from glob import glob
        from pkg_resources import parse_version

        self.vprint(1, 'Linking arcpy to new env')

        ARC_INSTALL_PREFIX = 'C:/Program Files (x86)/ArcGIS/Desktop'

        self.vprint(
            2,
            'Searching for most recent ArcGIS install '
            'in {}'.format(os.path.dirname(ARC_INSTALL_PREFIX)),
        )

        # maybe a more robust way to do this would be to
        # look in the registry for some ArcGIS keys?
        version = str(sorted([
            parse_version(
        ...
        self.vprint(2, 'Found version {} at {}'.format(
            version, ARC_INSTALL_PREFIX+version
        ))
    ...
    def create_conda_env(self, options):
        ...
        # been called from the ebagis manage.py, which has
        ...
        # add the desktop.pth file
        self.gen_arc_pth_file()
    ...
    def ebagis_command_error(self):
        ...
        print(
            ('Unfortunately, the install command must be run '
             ...
             'the ebagis command. Try running `python manage.py install`{}.'
             ).format(project_root_message)
        )

    def execute(self, *args, **options):
        if os.path.basename(sys.argv[0]) == 'ebagis':
            self.ebagis_command_error()
            return 1

        if options.get('version'):
            print('ebagis version {}'.format(self.get_version()))
            return 2
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        raise CommandError(
            'If you got here then you are not running the ebagis '
